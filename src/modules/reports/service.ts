...
export class ReportService {
  static async generate(sessionId: string, force = false) {
    ...
    const deterministicSummary = `Your HEAIR readiness result is ${Number(score.overallScore).toFixed(1)}/100 based on ${score.scoredResponseCount} scored responses. ${focusText}${strong.length ? ` while maintaining ${strong.map((x) => x.subDimension.label).join(", ")}` : ""}.`;
    ...
    const recommendations = [
      ...
      ...weak.map((x, i) => ({ category: "priority_action" as RecommendationCategory, subDimensionId: x.subDimensionId, title: `Improve ${x.subDimension.label}`, description: actions[x.subDimensionId] ?? "Create a focused improvement action for this HEAIR sub-dimension.", supportingCitationIds: [HEAIR_DOCUMENT_ID], sortOrder: i + 1 }))
    ];
