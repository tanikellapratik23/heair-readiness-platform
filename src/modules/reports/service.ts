...
export class ReportService {
  static async generate(sessionId: string, force = false) {
    ...
    const summary = `Your HEAIR readiness score is ${Number(score.overallScore).toFixed(1)}/100. Prioritize ${weak.map((x) => x.subDimension.label).join(", ")}; build on strengths in ${strong.map((x) => x.subDimension.label).join(", ")}.`;
