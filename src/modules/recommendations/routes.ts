...
export async function publicRecommendationRoutes(app: FastifyInstance) {
  ...
  app.get("/public/action-plan/export.csv", async (request, reply) => {
    ...
    return reply.type("text/csv; charset=utf-8").header("Content-Disposition", 'attachment; filename="heair-30-day-action-plan.csv"').send(lines);
  });
